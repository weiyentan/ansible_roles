	...
	if ($state -eq 'present')
	{
		if (!$evaltask)
		{
			if (-not ($check_mode))
			{
				...
				SCHTASKS /Create   /RU system /SC ONEVENT /EC System /MO *[System/EventID=101]  /TN Install_Patch /f /RL highest  /TR  "powershell.exe -noprofile  -executionpolicy bypass -file $env:windir\temp\Installpatch.ps1"
...
if ($name -eq "Monitor_Patch")
{
	if ($state -eq 'present')
	{
		if (!$evaltask)
		{
			if (-not ($check_mode))
			{
				SCHTASKS /Create  /RU system /SC  onstart /delay 0005:00 /TN Monitor_Patch /f /RL highest  /TR  "powershell.exe -noprofile -executionpolicy bypass -file $env:windir\temp\Patchmonitoring.ps1"
			}
			$result.changed = $true
